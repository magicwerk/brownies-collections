...
protected void doAssign(IByteList that) {
    ByteGapList list = (ByteGapList) that;
...
public ByteGapList getAll(int index, int len) {
    return (ByteGapList) super.getAll(index, len);
}

    @Override
public ByteGapList getAll(byte elem) {
    return (ByteGapList) super.getAll(elem);
}

    @Override
public <R> GapList<R> mappedList(IFunction<Byte, R> mapper) {
...
private void readObject(ObjectInputStream ois) throws IOException, ClassNotFoundException {
    ...
    @Override
public IByteList doCreate(int capacity) {
    if (capacity == -1) {
