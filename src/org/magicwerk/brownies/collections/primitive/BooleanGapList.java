...
public class BooleanGapList implements Cloneable, Serializable {
    ...
    // Cannot make a static reference to the non-static type boolean:
    // public static BooleanGapList EMPTY = BooleanGapList.create().unmodifiableList();
    // Syntax error:
    // public static  BooleanGapList EMPTY = BooleanGapList.create().unmodifiableList();

    /** Unmodifiable empty instance */
    @SuppressWarnings("rawtypes")
    private static BooleanGapList EMPTY = BooleanGapList.create().unmodifiableList();
     ...
     */
    static class ImmutableBooleanGapList extends BooleanGapList {
         ...
         */
        private ImmutableBooleanGapList(BooleanGapList that) {
            super(true, that);
        }
     ...
     */
    BooleanGapList(boolean copy, BooleanGapList that) {
        if (copy) {
	 ...
	 */
	public BooleanGapList(Collection<Boolean> that) {
		init(toArray(that));
	}
	 ...
	 * @param that	source object to copy
	 */
	public BooleanGapList(boolean... that) {
		init(that.clone());
	}
     ...
     *
     * @return a clone of this <tt>BooleanGapList</tt> instance
     */
	@SuppressWarnings("unchecked")
	...
	void init(boolean[] values) {
		doInit(values, values.length);
	}

	void init(int capacity) {
		doInit(new boolean[capacity], 0);
	}

	/**
	 * Initialize all instance fields.
	 *
	 * @param newValues	new values array to use (or (boolean)false for no change)
	 * @param empty		true to to set size to 0, otherwise size is set
	 * 					to values.length
	 */
	@SuppressWarnings("unchecked")
	private void doInit(boolean[] values, int size) {
    ...
    /**
     * Helper method for getting an element from the BooleanGapList.
    ...
    /**
     * Helper method for setting an element in the BooleanGapList.
	...
	/**
	 * Helper method for adding an element to the BooleanGapList.
	 ...
	 * @return      true if element has been added (BooleanGapList.add() will always return true)
	 */
	protected boolean doAdd(int index, boolean elem) {
	...
	private void moveData(int src, int dst, int len) {
		...
		// Write (boolean)false into array slots which are not used anymore
	...
	protected boolean doRemove(int index) {
		...
		boolean removed = values[physIdx];
		values[physIdx] = (boolean)false;
		size--;
    ...
    /**
     * Increases the capacity of this <tt>BooleanGapList</tt> instance, if
    ...
    /**
     * Trims the capacity of this <tt>BooleanGapList</tt> instance to be the
     * list's current size.  An application can use this operation to minimize
     * the storage of an <tt>BooleanGapList</tt> instance.
     */
	// Note: Provide this method to make transition from ArrayList as
	//       smooth as possible
    @SuppressWarnings("unchecked")
    ...
    public boolean equals(Object obj) {
    	...
    	@SuppressWarnings("unchecked")
		BooleanGapList list = (BooleanGapList) obj;
    	if (size != list.size()) {
    		return false;
    	}
	...
	/**
	 * Helper function to check two elements stored in the BooleanGapList
	 * for equality.
	 *
	 ...
	 */
	static boolean equalsElem(boolean val1, boolean val2) {
return val1 == val2;
	}
	 ...
	 */
	static int hashCodeElem(boolean val) {
return val ? 1231 : 1237;
    ...
    public boolean removeAll(BooleanGapList coll) {
    	// There is a special implementation accepting a BooleanGapList
	 ...
	 * @param  type of elements stored in the list
	 */
	private  void doToArray(boolean[] array, int index, int len) {
	...
	public boolean addAll(Collection<Boolean> coll) {
        ...
        @SuppressWarnings("unchecked")
        boolean[] array = (boolean[]) toArray(coll);
	...
	/**
     * Helper method for adding multiple elements to the BooleanGapList.
	 ...
	 */
	protected boolean doAddAll(int index, BooleanGapList list) {
     ...
     *             (each an <tt>boolean</tt>) in the proper order.
     * @param oos  output stream for serialization
     * @throws 	   IOException if serialization fails
     */
    private void writeboolean(ObjectOutputStream oos) throws IOException {
     ...
     */
    @SuppressWarnings("unchecked")
	private void readboolean(ObjectInputStream ois) throws IOException, ClassNotFoundException {
        // Read in array length and allocate array
        size = ois.readInt();
        values = (boolean[]) new boolean[size];

        // Read in all elements in the proper order.
        for (int i=0; i<size; i++) {
            values[i] = (boolean) ois.readBoolean();
        }
    }
    ...
    public static  void move(BooleanGapList src, int srcIndex, BooleanGapList dst, int dstIndex, int len) {
        if (src == dst) {
    		...
    		for (int i=0; i<len; i++) {
    			boolean elem = src.doSet(srcIndex+i, (boolean)false);
     ...
     * @throws 			IndexOutOfBoundsException if the ranges are invalid
     */
    public static  void copy(BooleanGapList src, int srcIndex, BooleanGapList dst, int dstIndex, int len) {
        if (src == dst) {
     ...
     * @throws 			IndexOutOfBoundsException if the ranges are invalid
     */
    public static  void swap(BooleanGapList src, int srcIndex, BooleanGapList dst, int dstIndex, int len) {
        if (src == dst) {
    ...
    public boolean[] getArray(int index, int len) {
        ...
        @SuppressWarnings("unchecked")
        boolean[] array = (boolean[]) new boolean[len];
        for (int i=0; i<len; i++) {
     ...
     */
    public void setAll(int index, BooleanGapList list) {
    	// There is a special implementation accepting a BooleanGapList
    	// so the method is also available in the primitive classes.
     ...
     * The elements which are moved away are set to (boolean)false.
     * Source and destination ranges may overlap.
     *
    ...
    public void move(int srcIndex, int dstIndex, int len) {
    	...
    	// Set elements to (boolean)false after the move operation
    	if (srcIndex < dstIndex) {
    		int fill = Math.min(len, dstIndex-srcIndex);
    		fill(srcIndex, fill, (boolean)false);
    	} else if (srcIndex > dstIndex) {
    		int fill = Math.min(len, srcIndex-dstIndex);
    		fill(srcIndex+len-fill, fill, (boolean)false);
    	}
    }
	...
	private void debugCheck() {
		...
		// Check that gap positions contain (boolean)false values
		if (gapSize > 0) {
			for (int i=gapStart; i<gapStart+gapSize; i++) {
				int pos = (i % values.length);
				assert(values[pos] == (boolean)false);
			}
		}

		// Check that all end positions contain (boolean)false values
		if (end != start) {
	 ...
	 * It is only used for debugging.
	 *
	 * @return	state in which BooleanGapList is
	 */
