...
public class ShortGapList implements Cloneable, Serializable {
    ...
    private static ShortGapList EMPTY = ShortGapList.create().unmodifiableList();
     ...
     */
    static class ImmutableShortGapList extends ShortGapList {
         ...
         */
        private ImmutableShortGapList(ShortGapList that) {
            super(true, that);
        }
	...
	public static  ShortGapList create(Collection<Short> elements) {
		return new ShortGapList(elements);
	}
	 ...
	 * @param  		type of elements stored in the list
	 */
	public static  ShortGapList create(short... elements) {
		return new ShortGapList(elements);
	}
     ...
     */
    ShortGapList(boolean copy, ShortGapList that) {
        if (copy) {
	...
	void init(short[] values) {
		doInit(values, values.length);
	}

	void init(int capacity) {
		doInit(new short[capacity], 0);
	}

	/**
	 * Initialize all instance fields.
	 *
	 * @param newValues	new values array to use (or (short)0 for no change)
	 * @param empty		true to to set size to 0, otherwise size is set
	 * 					to values.length
	 */
	@SuppressWarnings("unchecked")
	private void doInit(short[] values, int size) {
...
return (int) val;
	 ...
	 * @param  type of elements stored in the list
	 */
	private  void doToArray(short[] array, int index, int len) {
	 ...
	 */
	protected boolean doAddAll(int index, ShortGapList list) {
    ...
    public static  void move(ShortGapList src, int srcIndex, ShortGapList dst, int dstIndex, int len) {
        if (src == dst) {
    		...
    		for (int i=0; i<len; i++) {
    			short elem = src.doSet(srcIndex+i, (short)0);
     ...
     */
    public ShortGapList get(int index, int len) {
    ...
    */
    ...
    public  int binarySearch(int index, int len, short key) {
    	...
    	normalize();
    	return Arrays.binarySearch((short[]) values, index, index+len, key);
    }
