 ...
 */
public class DoubleGapList implements Cloneable, Serializable {
    ...
    // Cannot make a static reference to the non-static type double:
    // public static DoubleGapList EMPTY = DoubleGapList.create().unmodifiableList();
    // Syntax error:
    // public static  DoubleGapList EMPTY = DoubleGapList.create().unmodifiableList();

    /** Unmodifiable empty instance */
    @SuppressWarnings("rawtypes")
    private static DoubleGapList EMPTY = DoubleGapList.create().unmodifiableList();

    /**
     * @return unmodifiable empty instance
     */
    @SuppressWarnings("unchecked")
    public static  DoubleGapList EMPTY() {
        return EMPTY;
    }
     ...
     */
    static class ImmutableDoubleGapList extends DoubleGapList {
         ...
         */
        private ImmutableDoubleGapList(DoubleGapList that) {
            super(true, that);
        }
    ...
    public static  DoubleGapList create(int capacity) {
        return new DoubleGapList(capacity);
    }

	public static  DoubleGapList create(Collection<Double> elements) {
		return new DoubleGapList(elements);
	}
	 ...
	 * @param  		type of elements stored in the list
	 */
	public static  DoubleGapList create(double... elements) {
		return new DoubleGapList(elements);
	}
     ...
     */
    DoubleGapList(boolean copy, DoubleGapList that) {
        if (copy) {
     ...
     *
     * @return a clone of this <tt>DoubleGapList</tt> instance
     */
	@SuppressWarnings("unchecked")
	...
	void init(int capacity) {
		doInit(new double[capacity], 0);
	}

	/**
	 * Initialize all instance fields.
	 *
	 * @param newValues	new values array to use (or (double)0 for no change)
	 * @param empty		true to to set size to 0, otherwise size is set
	 * 					to values.length
	 */
	@SuppressWarnings("unchecked")
	private void doInit(double[] values, int size) {
    ...
    /**
     * Helper method for getting an element from the DoubleGapList.
    ...
    /**
     * Helper method for setting an element in the DoubleGapList.
	...
	/**
	 * Helper method for adding an element to the DoubleGapList.
	...
	private void moveData(int src, int dst, int len) {
		...
		// Write (double)0 into array slots which are not used anymore
    ...
    public void ensureCapacity(int minCapacity) {
		...
		if (start == 0) {
			...
			newValues = (double []) new double[newCapacity];
    ...
    /**
     * Trims the capacity of this <tt>DoubleGapList</tt> instance to be the
     * list's current size.  An application can use this operation to minimize
     * the storage of an <tt>DoubleGapList</tt> instance.
     */
	// Note: Provide this method to make transition from ArrayList as
	//       smooth as possible
    @SuppressWarnings("unchecked")
	...
	/**
	 * Helper function to check two elements stored in the DoubleGapList
	 * for equality.
	 *
	 ...
	 */
	static boolean equalsElem(double val1, double val2) {
return Double.compare(val1, val2) == 0;
	 ...
	 */
	static int hashCodeElem(double val) {
return (int) val;
	}
     ...
     */
    public boolean removeAll(DoubleGapList coll) {
    	// There is a special implementation accepting a DoubleGapList
     ...
     */
    public boolean retainAll(DoubleGapList coll) {
    	// There is a special implementation accepting a DoubleGapList
    	// so the method is also available in the primitive classes.
	    boolean modified = false;
		for (int i=0; i<size; i++) {
	 ...
	 * @param  type of elements stored in the list
	 */
	private  void doToArray(double[] array, int index, int len) {
     ...
     * @throws NullPointerException if the specified collection is (double)0
     */
	...
	public boolean addAll(Collection<Double> coll) {
        ...
        @SuppressWarnings("unchecked")
        double[] array = (double[]) toArray(coll);
     ...
     * @return <tt>true</tt> if this list changed as a result of the call
     * @throws NullPointerException if the specified list is (double)0
     */
    public boolean addAll(DoubleGapList list) {
     ...
     * @throws NullPointerException if the specified collection is (double)0
     */
	public boolean addAll(int index, DoubleGapList list) {
		checkIndexAdd(index);
	...
	/**
     * Helper method for adding multiple elements to the DoubleGapList.
	 ...
	 */
	protected boolean doAddAll(int index, DoubleGapList list) {
     ...
     *             (each an <tt>double</tt>) in the proper order.
     * @param oos  output stream for serialization
     * @throws 	   IOException if serialization fails
     */
    private void writedouble(ObjectOutputStream oos) throws IOException {
     ...
     */
    @SuppressWarnings("unchecked")
	private void readdouble(ObjectInputStream ois) throws IOException, ClassNotFoundException {
        // Read in array length and allocate array
        size = ois.readInt();
        values = (double[]) new double[size];

        // Read in all elements in the proper order.
        for (int i=0; i<size; i++) {
            values[i] = (double) ois.readDouble();
        }
    }
     ...
     */
    public static  void move(DoubleGapList src, int srcIndex, DoubleGapList dst, int dstIndex, int len) {
        if (src == dst) {
    		...
    		for (int i=0; i<len; i++) {
    			double elem = src.doSet(srcIndex+i, (double)0);
     ...
     * @throws 			IndexOutOfBoundsException if the ranges are invalid
     */
    public static  void copy(DoubleGapList src, int srcIndex, DoubleGapList dst, int dstIndex, int len) {
        if (src == dst) {
     ...
     * @throws 			IndexOutOfBoundsException if the ranges are invalid
     */
    public static  void swap(DoubleGapList src, int srcIndex, DoubleGapList dst, int dstIndex, int len) {
        if (src == dst) {
     ...
     * @return      DoubleGapList containing the specified range of elements from list
     */
    public DoubleGapList get(int index, int len) {
        checkRange(index, len);

        DoubleGapList list = new DoubleGapList(len);
        for (int i=0; i<len; i++) {
    ...
    public double[] getArray(int index, int len) {
        ...
        @SuppressWarnings("unchecked")
        double[] array = (double[]) new double[len];
        for (int i=0; i<len; i++) {
     ...
     */
    public void setAll(int index, DoubleGapList list) {
    	// There is a special implementation accepting a DoubleGapList
    	// so the method is also available in the primitive classes.
     ...
     * The elements which are moved away are set to (double)0.
     * Source and destination ranges may overlap.
     *
    ...
    public void move(int srcIndex, int dstIndex, int len) {
    	...
    	// Set elements to (double)0 after the move operation
    	if (srcIndex < dstIndex) {
    		int fill = Math.min(len, dstIndex-srcIndex);
    		fill(srcIndex, fill, (double)0);
    	} else if (srcIndex > dstIndex) {
    		int fill = Math.min(len, srcIndex-dstIndex);
    		fill(srcIndex+len-fill, fill, (double)0);
    	}
    }
     ...
     * 						((double)0 means the elements natural ordering should be used)
     *
     * @see Arrays#sort
     */
    public void sort() {
    ...
    Answer:
       ...
       the type stored in the DoubleGapList, but also with any other type you
       ...
       new DoubleGapList<Name>().binarySearch(new Name("a"), new NameComparator());
       new DoubleGapList<Name>().binarySearch("a), new NameComparator());
       ...
       static class NameComparator implements Comparator<double> {
    ...
    */
     ...
     *                      A <tt>(double)0</tt> value indicates that the elements'
    ...
    public  int binarySearch(int index, int len, double key) {
    	...
    	normalize();
    	return Arrays.binarySearch((double[]) values, index, index+len, key);
    }
	...
	private void debugCheck() {
		...
		// Check that gap positions contain (double)0 values
		if (gapSize > 0) {
			for (int i=gapStart; i<gapStart+gapSize; i++) {
				int pos = (i % values.length);
				assert(values[pos] == (double)0);
			}
		}

		// Check that all end positions contain (double)0 values
		if (end != start) {
	 ...
	 * It is only used for debugging.
	 *
	 * @return	state in which DoubleGapList is
	 */
	 ...
	 */
	private String debugPrint(double[] values) {
		StringBuilder buf = new StringBuilder();
		buf.append("[ ");
		for (int i=0; i<values.length; i++) {
			if (i > 0) {
