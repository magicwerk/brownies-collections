...
public class DoubleGapList implements Cloneable, Serializable {
	...
	// Guide to subclass DoubleGapList
	...
	public void init(int capacity) {
		init(new double[capacity], 0);
	}

	public void init(Collection<Double> coll) {
		double[] array = toArray(coll);
		init(array, array.length);
	}

	public void init(double... elems) {
		double[] array = elems.clone();
		init(array, array.length);
	}
...
return (int) val;
    ...
    */
    ...
    public  int binarySearch(int index, int len, double key) {
    	...
    	normalize();
    	return ArraysHelper.binarySearch((double[]) values, index, index+len, key);
    }
