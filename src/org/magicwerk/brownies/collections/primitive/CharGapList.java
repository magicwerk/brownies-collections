 ...
 */
public class CharGapList implements Cloneable, Serializable {
    ...
    // Cannot make a static reference to the non-static type char:
    ...
    private static CharGapList EMPTY = CharGapList.create().unmodifiableList();
     ...
     */
    static class ImmutableCharGapList extends CharGapList {
         ...
         */
        private ImmutableCharGapList(CharGapList that) {
            super(true, that);
        }
	...
	public static  CharGapList create(Collection<Character> elements) {
		return new CharGapList(elements);
	}
	 ...
	 * @param  		type of elements stored in the list
	 */
	public static  CharGapList create(char... elements) {
		return new CharGapList(elements);
	}
     ...
     */
    CharGapList(boolean copy, CharGapList that) {
        if (copy) {
	 ...
	 */
	public CharGapList(Collection<Character> that) {
		init(toArray(that));
	}
    ...
    /**
     * Returns a shallow copy of this <tt>CharGapList</tt> instance
     * (the new list will contain the same elements as the source list, i.e. the elements themselves are not copied).
     * This method is identical to clone() except that the result is casted to CharGapList.
     *
     ...
     *
     * @return a clone of this <tt>CharGapList</tt> instance
     */
	@SuppressWarnings("unchecked")
    
    public Object clone() {
		try {
			CharGapList list = (CharGapList) super.clone();
	...
	/**
	 * Normalize data of CharGapList so the elements are found
	...
	void init(int capacity) {
		doInit(new char[capacity], 0);
	}

	/**
	 * Initialize all instance fields.
	 *
	 * @param newValues	new values array to use (or (char)0 for no change)
	 * @param empty		true to to set size to 0, otherwise size is set
	 * 					to values.length
	 */
	@SuppressWarnings("unchecked")
	private void doInit(char[] values, int size) {
    ...
    /**
     * Helper method for getting an element from the CharGapList.
    ...
    /**
     * Helper method for setting an element in the CharGapList.
	...
	/**
	 * Helper method for adding an element to the CharGapList.
	 ...
	 * @return      true if element has been added (CharGapList.add() will always return true)
	 */
	protected boolean doAdd(int index, char elem) {
    ...
    /**
     * Increases the capacity of this <tt>CharGapList</tt> instance, if
    ...
    /**
     * Trims the capacity of this <tt>CharGapList</tt> instance to be the
     * list's current size.  An application can use this operation to minimize
     * the storage of an <tt>CharGapList</tt> instance.
     */
	// Note: Provide this method to make transition from ArrayList as
	//       smooth as possible
    @SuppressWarnings("unchecked")
	...
	/**
	 * Helper function to check two elements stored in the CharGapList
	 * for equality.
	 *
	 ...
	 */
	static boolean equalsElem(char val1, char val2) {
return val1 == val2;
	}
	 ...
	 */
	static int hashCodeElem(char val) {
return (int) val;
	}
    ...
    public boolean removeAll(CharGapList coll) {
    	// There is a special implementation accepting a CharGapList
	 ...
	 * @param  type of elements stored in the list
	 */
	private  void doToArray(char[] array, int index, int len) {
     ...
     * @throws NullPointerException if the specified collection is (char)0
     */
     ...
     * @return <tt>true</tt> if this list changed as a result of the call
     * @throws NullPointerException if the specified list is (char)0
     */
    public boolean addAll(CharGapList list) {
     ...
     * @throws NullPointerException if the specified collection is (char)0
     */
	public boolean addAll(int index, CharGapList list) {
		checkIndexAdd(index);
	...
	/**
     * Helper method for adding multiple elements to the CharGapList.
	 ...
	 */
	protected boolean doAddAll(int index, CharGapList list) {
     ...
     *             (each an <tt>char</tt>) in the proper order.
     * @param oos  output stream for serialization
     * @throws 	   IOException if serialization fails
     */
    private void writechar(ObjectOutputStream oos) throws IOException {
     ...
     */
    @SuppressWarnings("unchecked")
	private void readchar(ObjectInputStream ois) throws IOException, ClassNotFoundException {
    ...
    public static  void move(CharGapList src, int srcIndex, CharGapList dst, int dstIndex, int len) {
        if (src == dst) {
    		...
    		for (int i=0; i<len; i++) {
    			char elem = src.doSet(srcIndex+i, (char)0);
     ...
     * @throws 			IndexOutOfBoundsException if the ranges are invalid
     */
    public static  void copy(CharGapList src, int srcIndex, CharGapList dst, int dstIndex, int len) {
        if (src == dst) {
     ...
     * @throws 			IndexOutOfBoundsException if the ranges are invalid
     */
    public static  void swap(CharGapList src, int srcIndex, CharGapList dst, int dstIndex, int len) {
        if (src == dst) {
     ...
     * @return      CharGapList containing the specified range of elements from list
     */
    public CharGapList get(int index, int len) {
        checkRange(index, len);

        CharGapList list = new CharGapList(len);
        for (int i=0; i<len; i++) {
     ...
     */
    public void setAll(int index, CharGapList list) {
    	// There is a special implementation accepting a CharGapList
    	// so the method is also available in the primitive classes.
     ...
     * The elements which are moved away are set to (char)0.
     * Source and destination ranges may overlap.
     *
     ...
     * 						((char)0 means the elements natural ordering should be used)
     *
     * @see Arrays#sort
     */
    public void sort() {
    ...
    Answer:
       ...
       the type stored in the CharGapList, but also with any other type you
       ...
       new CharGapList<Name>().binarySearch(new Name("a"), new NameComparator());
       new CharGapList<Name>().binarySearch("a), new NameComparator());
       ...
       static class NameComparator implements Comparator<char> {
    ...
    */
     ...
     *                      A <tt>(char)0</tt> value indicates that the elements'
    ...
    public  int binarySearch(int index, int len, char key) {
    	...
    	normalize();
    	return Arrays.binarySearch((char[]) values, index, index+len, key);
    }
