...
public class FloatGapList implements Cloneable, Serializable {
    ...
    // Cannot make a static reference to the non-static type float:
    ...
    private static FloatGapList EMPTY = FloatGapList.create().unmodifiableList();
     ...
     */
    static class ImmutableFloatGapList extends FloatGapList {
         ...
         */
        private ImmutableFloatGapList(FloatGapList that) {
            super(true, that);
        }
	...
	public static  FloatGapList create(Collection<Float> elements) {
		return new FloatGapList(elements);
	}
	 ...
	 * @param  		type of elements stored in the list
	 */
	public static  FloatGapList create(float... elements) {
		return new FloatGapList(elements);
	}
     ...
     */
    FloatGapList(boolean copy, FloatGapList that) {
        if (copy) {
	 ...
	 */
	public FloatGapList(Collection<Float> that) {
		init(toArray(that));
	}
	 ...
	 * @param that	source object to copy
	 */
	public FloatGapList(float... that) {
		init(that.clone());
	}
     ...
     *
     * @return a clone of this <tt>FloatGapList</tt> instance
     */
	@SuppressWarnings("unchecked")
    
    public Object clone() {
		try {
			FloatGapList list = (FloatGapList) super.clone();
	...
	void init(int capacity) {
		doInit(new float[capacity], 0);
	}

	/**
	 * Initialize all instance fields.
	 *
	 * @param newValues	new values array to use (or (float)0 for no change)
	 * @param empty		true to to set size to 0, otherwise size is set
	 * 					to values.length
	 */
	@SuppressWarnings("unchecked")
	private void doInit(float[] values, int size) {
    ...
    /**
     * Helper method for getting an element from the FloatGapList.
    ...
    /**
     * Helper method for setting an element in the FloatGapList.
	...
	/**
	 * Helper method for adding an element to the FloatGapList.
	...
	private void moveData(int src, int dst, int len) {
		...
		// Write (float)0 into array slots which are not used anymore
    ...
    /**
     * Increases the capacity of this <tt>FloatGapList</tt> instance, if
    ...
    /**
     * Trims the capacity of this <tt>FloatGapList</tt> instance to be the
     * list's current size.  An application can use this operation to minimize
     * the storage of an <tt>FloatGapList</tt> instance.
     */
	// Note: Provide this method to make transition from ArrayList as
	//       smooth as possible
    @SuppressWarnings("unchecked")
    ...
    public boolean equals(Object obj) {
    	...
    	@SuppressWarnings("unchecked")
		FloatGapList list = (FloatGapList) obj;
    	if (size != list.size()) {
    		return false;
    	}
	...
	/**
	 * Helper function to check two elements stored in the FloatGapList
	 * for equality.
	 *
	 ...
	 */
	static boolean equalsElem(float val1, float val2) {
return Float.compare(val1, val2) == 0;
	 ...
	 */
	static int hashCodeElem(float val) {
return (int) val;
	}
    ...
    public boolean removeAll(FloatGapList coll) {
    	// There is a special implementation accepting a FloatGapList
	 ...
	 * @param  type of elements stored in the list
	 */
	private  void doToArray(float[] array, int index, int len) {
     ...
     * @throws NullPointerException if the specified collection is (float)0
     */
	...
	public boolean addAll(Collection<Float> coll) {
        ...
        @SuppressWarnings("unchecked")
        float[] array = (float[]) toArray(coll);
     ...
     * @return <tt>true</tt> if this list changed as a result of the call
     * @throws NullPointerException if the specified list is (float)0
     */
    public boolean addAll(FloatGapList list) {
     ...
     * @throws NullPointerException if the specified collection is (float)0
     */
	public boolean addAll(int index, FloatGapList list) {
		checkIndexAdd(index);
	...
	/**
     * Helper method for adding multiple elements to the FloatGapList.
	 ...
	 */
	protected boolean doAddAll(int index, FloatGapList list) {
     ...
     *             (each an <tt>float</tt>) in the proper order.
     * @param oos  output stream for serialization
     * @throws 	   IOException if serialization fails
     */
    private void writefloat(ObjectOutputStream oos) throws IOException {
     ...
     */
    @SuppressWarnings("unchecked")
	private void readfloat(ObjectInputStream ois) throws IOException, ClassNotFoundException {
    ...
    public static  void move(FloatGapList src, int srcIndex, FloatGapList dst, int dstIndex, int len) {
        if (src == dst) {
    		...
    		for (int i=0; i<len; i++) {
    			float elem = src.doSet(srcIndex+i, (float)0);
     ...
     * @throws 			IndexOutOfBoundsException if the ranges are invalid
     */
    public static  void copy(FloatGapList src, int srcIndex, FloatGapList dst, int dstIndex, int len) {
        if (src == dst) {
     ...
     * @throws 			IndexOutOfBoundsException if the ranges are invalid
     */
    public static  void swap(FloatGapList src, int srcIndex, FloatGapList dst, int dstIndex, int len) {
        if (src == dst) {
     ...
     * @return      FloatGapList containing the specified range of elements from list
     */
    public FloatGapList get(int index, int len) {
        checkRange(index, len);

        FloatGapList list = new FloatGapList(len);
        for (int i=0; i<len; i++) {
     ...
     */
    public void setAll(int index, FloatGapList list) {
    	// There is a special implementation accepting a FloatGapList
    	// so the method is also available in the primitive classes.
     ...
     * The elements which are moved away are set to (float)0.
     * Source and destination ranges may overlap.
     *
    ...
    public void move(int srcIndex, int dstIndex, int len) {
    	...
    	// Set elements to (float)0 after the move operation
    	if (srcIndex < dstIndex) {
    		int fill = Math.min(len, dstIndex-srcIndex);
    		fill(srcIndex, fill, (float)0);
    	} else if (srcIndex > dstIndex) {
    		int fill = Math.min(len, srcIndex-dstIndex);
    		fill(srcIndex+len-fill, fill, (float)0);
    	}
    }
     ...
     * 						((float)0 means the elements natural ordering should be used)
     *
     * @see Arrays#sort
     */
    public void sort() {
    ...
    Answer:
       ...
       the type stored in the FloatGapList, but also with any other type you
       ...
       new FloatGapList<Name>().binarySearch(new Name("a"), new NameComparator());
       new FloatGapList<Name>().binarySearch("a), new NameComparator());
       ...
       static class NameComparator implements Comparator<float> {
    ...
    */
     ...
     *                      A <tt>(float)0</tt> value indicates that the elements'
    ...
    public  int binarySearch(int index, int len, float key) {
    	...
    	normalize();
    	return Arrays.binarySearch((float[]) values, index, index+len, key);
    }
	...
	private void debugCheck() {
		...
		// Check that gap positions contain (float)0 values
		if (gapSize > 0) {
			for (int i=gapStart; i<gapStart+gapSize; i++) {
				int pos = (i % values.length);
				assert(values[pos] == (float)0);
			}
		}

		// Check that all end positions contain (float)0 values
		if (end != start) {
