...
private int getFloatBlockIndex(int index, boolean write, int modify) {
    ...
    return getFloatBlockIndex2(index, write, modify);
}

    private int getFloatBlockIndex2(int index, boolean write, int modify) {
    if (index == size) {
...
private void doGetFloatBlock(int index, int modify) {
    ...
    if (currNode.relPos == 0) {
        ...
        while (true) {
            ...
            if (index < currFloatBlockEnd) {
                ...
                wasLeft = doGetFloatBlockLeft(modify, nextNode, wasLeft);
                if (nextNode == null) {
                    break;
                }
            } else {
                ...
                wasLeft = doGetFloatBlockRight(modify, nextNode, wasLeft);
                if (nextNode == null) {
                    break;
                }
            }
    ...
    private boolean doGetFloatBlockLeft(int modify, FloatBlockNode nextNode, boolean wasLeft) {
    if (modify != 0) {
    ...
    private boolean doGetFloatBlockRight(int modify, FloatBlockNode nextNode, boolean wasLeft) {
    if (modify != 0) {
        if (nextNode == null || wasLeft) {
            if (currNode.relPos > 0) {
                currNode.relPos += modify;
...
protected boolean doAdd(int index, float element) {
    ...
    if (currNode.block.size() < maxSize || (currNode.block.size() == 1 && currNode.block.size() < blockSize)) {
        ...
        if (index == size) {
            ...
            doAddSplitFloatBlock(index, element, pos, newFloatBlock);
        }
    }
    size++;
    if (CHECK)
    ...
    private void doAddSplitFloatBlock(int index, float element, int pos, FloatBlock newFloatBlock) {
...
private void modify(FloatBlockNode node, int modify) {
    ...
    private void modifyLeftNode(FloatBlockNode node, int modify) {
    ...
    private void modifyRightNode(FloatBlockNode node, int modify) {
    node.relPos += modify;
    FloatBlockNode leftNode = node.getLeftSubTree();
    if (leftNode != null) {
        leftNode.relPos -= modify;
...
protected boolean doAddAll(int index, IFloatListable list) {
    ...
    private void doAddAllTail(IFloatListable list, int addPos, int addLen, int space) {
    ...
    private void doAddAllHead(IFloatListable list, int addPos, int addLen, int space) {
    assert (addPos == 0);
    for (int i = 0; i < space; i++) {
...
void doAddAllMiddle(IFloatListable list, int addPos) {
    ...
    if (has < should) {
        ...
        int add = should - has;
        IFloatList sublist = list2.getAll(0, add);
        listPos += add;
    ...
    while (numFloatBlocks > 0) {
        ...
        assert (add > 0);
        IFloatList sublist = list2.getAll(listPos, add);
        listPos += add;
...
protected void doRemoveAll(int index, int len) {
    ...
    private void doRemoveAll2(int index, int len, int startPos, FloatBlockNode startNode, FloatBlockNode endNode) {
    if (CHECK)
        check();
