...
public class FloatMergeSort {
	...
	public static <E> void sort(IFloatList list) {
		FloatMergeSort sort = new FloatMergeSort(list);
		sort.sort();
	}

	public static <E> void sort(IFloatList list, int from, int to) {
		FloatMergeSort sort = new FloatMergeSort(list);
		sort.sort(from, to);
	}

	private FloatMergeSort(IFloatList list) {
		this.list = list;
	...
	private int compare(int idx1, int idx2) {
		float val1 = list.get(idx1);
		float val2 = list.get(idx2);
		return (val1<val2 ? -1 : (val1==val2 ? 0 : 1));
	}

	private void swap(int idx1, int idx2) {
		float val = list.get(idx1);
