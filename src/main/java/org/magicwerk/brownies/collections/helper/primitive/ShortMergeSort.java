 ...
 * @author Thomas Mauch
 */
public class ShortMergeSort {

    IShortList list;

    public static <E> void sort(IShortList list) {
        ShortMergeSort sort = new ShortMergeSort(list);
        sort.sort();
    }

    public static <E> void sort(IShortList list, int from, int to) {
        ShortMergeSort sort = new ShortMergeSort(list);
        sort.sort(from, to);
    }

    private ShortMergeSort(IShortList list) {
        this.list = list;
    }
    ...
    private int compare(int idx1, int idx2) {
        short val1 = list.get(idx1);
        short val2 = list.get(idx2);
        return (val1 < val2 ? -1 : (val1 == val2 ? 0 : 1));
    }

    private void swap(int idx1, int idx2) {
        short val = list.get(idx1);
